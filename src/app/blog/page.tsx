...
const BlogPost = memo(({ post }: { post: BlogPost }) => (
  <Link
    key={post.slug}
    href={`/blog/${post.slug}`}
    className="group relative overflow-hidden rounded-xl border border-black/5 hover:border-black/10 dark:border-white/5 dark:hover:border-white/10 p-4 shadow-md hover:shadow-lg dark:hover:shadow-white/10 transition-all duration-200"
  >
      ...
      <div className="flex items-center gap-2">
        <div className="flex flex-wrap gap-1.5">
          {post.tags.map((tag) => (
            <div
              key={tag}
              className="rounded-full bg-gray-100 dark:bg-gray-800 px-2 py-0.5 text-gray-700 dark:text-gray-300"
            >
              {tag}
            </div>
          ))}
