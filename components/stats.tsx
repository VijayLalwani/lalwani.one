...
import { departureMono } from '../styles/fonts';
...
export function Stats() {
  ...
  return (
    <div className={`flex select-none flex-col gap-1 ${departureMono.className}`}>
      <h3 className="text-sm font-semibold text-mauve-50 dark:text-evuam-50">
        ░ STATS
      </h3>
...
function StatRow({ label, value, hoverContent, color }: { 
  label: string;
  value: string;
  hoverContent?: string[];
  color: string;
      ...
      >
        ...
        {hoverContent && (
          <div className={`${isActive ? 'block' : 'hidden'} group-hover:block`}>
            <div className="flex flex-col bg-mauve-50 shadow-md md:flex-row md:items-center md:bg-transparent md:shadow-none dark:bg-evuam-50 md:dark:bg-transparent">
              <span className="hidden md:inline pl-1">¦</span>
              
              {hoverContent.map((content, index) => (
                <Fragment key={content}>
                  {index > 0 && <span className="hidden md:inline px-1">¦</span>}
